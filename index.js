...
function getAverageGoals(data) {
    let homeGoalsAve = data.reduce((total, item) => {
        return total += item['Home Team Goals'];
    }, 0);
    let awayGoalsAve = data.reduce((total, item) => {
        return total += item['Away Team Goals'];
    }, 0);
    homeGoalsAve /= data.length;
    awayGoalsAve /= data.length;
    console.log(`Home goals average: ${homeGoalsAve} - Away goals average: ${awayGoalsAve}`)
};
